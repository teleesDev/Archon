...
from database.database import (
    adicionar_cliente,
    atualizar_cliente_completo,
    buscar_clientes,
    buscar_processos,
    deletar_cliente,
    obter_nome_cliente_por_id,
...
def _resource_path(relative_path):
    ...
    if getattr(sys, "frozen", False):
        ...
        # __file__ está em modules/ — assets ficam na pasta pai (raiz do projeto)
...
def criar_aba_clientes(app, parent_frame):
    ...
    app.frame_lista_clientes.grid_rowconfigure(0, weight=0)
    ...
    app.entry_busca_cliente = ctk.CTkEntry(topo_frame, height=35, border_width=1, border_color=("gray80", "gray30"))
    app.entry_busca_cliente.insert(0, "Buscar por Nome, CPF ou CNPJ...")
    app.entry_busca_cliente.configure(text_color="gray")
    ...
    btn_novo_cliente = ctk.CTkButton(topo_frame, text="+ Cadastrar Novo", width=150, height=35,
                                     ...
                                     fg_color="#21354E", hover_color="#152436", text_color="#FFFFFF",
    ...
    _card_cli = ctk.CTkFrame(app.frame_lista_clientes, fg_color="#FFFFFF",
                              corner_radius=15, border_width=1, border_color="#E0E3E8")
    _card_cli.grid(row=2, column=0, sticky="nsew", padx=20, pady=(0, 20))
    _card_cli.grid_rowconfigure(0, weight=1)
    _card_cli.grid_columnconfigure(0, weight=1)

    app.tabs_pf_pj = ctk.CTkTabview(_card_cli, fg_color="transparent",
                                      bg_color="#FFFFFF", corner_radius=12)
    app.tabs_pf_pj.grid(row=0, column=0, sticky="nsew") 
    ...
    for aba in abas_clientes:
        bloco = ctk.CTkFrame(header_customizado, fg_color=("gray85", "gray15"), corner_radius=6)
    ...
    app.frame_form_cliente = ctk.CTkFrame(parent_frame, fg_color=["#FFFFFF", "#1E1E1E"])
    app.frame_form_cliente.grid_rowconfigure(0, weight=1)
    app.frame_form_cliente.grid_columnconfigure(0, weight=1)

    scroll_form_box = ctk.CTkScrollableFrame(app.frame_form_cliente, fg_color=["#FFFFFF", "#1E1E1E"])
    ...
    try:
        img_voltar = ctk.CTkImage(Image.open(_resource_path("assets/icone_voltar.png")), size=(20, 20))
    except Exception:
        img_voltar = None

    btn_voltar = ctk.CTkButton(app.form_box, text="←" if not img_voltar else "", image=img_voltar, width=30, height=30, 
                               fg_color="transparent", hover_color="gray85", text_color="black",
    ...
    try:
        img_form = ctk.CTkImage(Image.open(_resource_path("assets/icone_formulario.png")), size=(24, 24))
    except Exception:
        img_form = None
    
    if img_form: ctk.CTkLabel(frame_titulo, text="", image=img_form).pack(side="left", padx=(15, 10))
    ctk.CTkLabel(frame_titulo, text="Registro de Cliente", font=ctk.CTkFont(size=22, weight="bold"), text_color="black").pack(side="left")
    ...
    ctk.CTkRadioButton(frame_radios, text="Pessoa Física", font=ctk.CTkFont(weight="bold"), text_color="black", variable=app.var_tipo_pessoa, value="Pessoa Física", command=lambda: alternar_campos_pj(app)).pack(side="left", padx=(40, 15), pady=(40, 0))
    ctk.CTkRadioButton(frame_radios, text="Pessoa Jurídica", font=ctk.CTkFont(weight="bold"), text_color="black", variable=app.var_tipo_pessoa, value="Pessoa Jurídica", command=lambda: alternar_campos_pj(app)).pack(side="left", padx=15, pady=(40, 0))
    ...
    app.btn_salvar_cliente = ctk.CTkButton(app.form_box, text="Salvar Cliente", font=ctk.CTkFont(weight="bold"), 
                                           fg_color="#D4AF37", hover_color="#B08D45", text_color="#1A1A1A", 
...
def atualizar_lista_clientes(app, event=None):
    ...
    img_proc_item = _ic("icon_processo_item.png", (16,16))
    ...
    for c in clientes_db:
        try:
            ...
            cor_accent = "#D4AF37" if is_pj else "#21354E" 

            # AJUSTE DE HARMONIA: Cards com fundo mais claro (#2B2B31) para diferenciar do fundo geral
            card = ctk.CTkFrame(parent, corner_radius=10, height=65, fg_color=("#F8F9FA", "#F8F9FA"), border_width=1, border_color=("gray85", "#3A3A42"))
            ...
            btn_exp = ctk.CTkButton(btn_frame, text="", image=img_seta, width=28, height=28, fg_color="transparent", hover_color=("gray80", "gray30"))
            btn_exp.pack(side="left", padx=1)
            ctk.CTkButton(btn_frame, text="", image=img_edit, width=28, height=28, fg_color="transparent", hover_color=("gray80", "gray30"), command=lambda d=c: ir_para_editar(app, d)).pack(side="left", padx=1)
            ctk.CTkButton(btn_frame, text="", image=img_del, width=28, height=28, fg_color="transparent", text_color="#d93838", hover_color=("gray80", "gray30"), command=lambda i=id_c: confirmar_exclusao_cliente(app, i)).pack(side="left", padx=1)
            ...
            if procs_cliente:
                ...
                for p in procs_cliente:
                    ...
                    ctk.CTkFrame(p_grid, 1, height=1, fg_color=("gray80", "gray30")).grid(row=0, column=1, sticky="ns", pady=15)
                    ...
                    ctk.CTkFrame(p_grid, width=1, height=1, fg_color=("gray80", "gray30")).grid(row=0, column=3, sticky="ns", pady=15)
...
def confirmar_exclusao_cliente(app, id_cliente):
    ...
    ctk.CTkButton(frame_botoes, text="Sim", fg_color="#2ecc71", hover_color="#27ae60", text_color="black", width=100, command=sim).pack(side="left", padx=10)
    ctk.CTkButton(frame_botoes, text="Não", fg_color="#e74c3c", hover_color="#c0392b", text_color="white", width=100, command=modal.destroy).pack(side="left", padx=10)
...
def remover_cliente(app, id_cliente):
    ...
    app._processos_desatualizados = True
    atualizar_lista_clientes(app)
    if hasattr(processos, 'atualizar_lista_processos'):
        processos.atualizar_lista_processos(app)
