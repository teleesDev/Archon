...
def _icone(nome, tam):
    try:
        return ctk.CTkImage(Image.open(_resource_path(f"assets/{nome}")), size=tam)
    except Exception:
        return None
...
def _deve_alertar_backup():
    """Retorna True se hoje é um dos últimos 3 dias do mês E backup do mês ainda não foi feito."""
    hoje = datetime.now()
    ultimo_dia = calendar.monthrange(hoje.year, hoje.month)[1]
    if hoje.day < ultimo_dia - 2:   # só nos 3 últimos dias
        return False
    # Verificar se já existe backup deste mês na pasta backups/
    try:
        base = os.path.dirname(os.path.abspath(__file__))
        backup_dir = os.path.join(os.path.dirname(base), "backups")
        mes_str = hoje.strftime("%m-%Y")   # ex: "05-2026"
        if os.path.isdir(backup_dir):
            for f in os.listdir(backup_dir):
                if f.startswith("backup_") and mes_str in f:
                    return False   # backup do mês já existe
    except:
        pass
    return True
...
def criar_dashboard(app, parent_frame):
    ...
    # Banner de backup mensal — aparece nos últimos 3 dias do mês
    if _deve_alertar_backup():
        pill_bkp = ctk.CTkFrame(frame_alerta, fg_color="#FFF3CD", corner_radius=20,
                                border_width=1, border_color="#C9A84C")
        pill_bkp.pack(side="left", padx=(0, 10))
        ctk.CTkLabel(pill_bkp,
                     text="⚠  Backup mensal recomendado!",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color="#7A5000").pack(padx=14, pady=6)

    if eventos_hoje:
        _btn(frame_alerta,
             f"  {len(eventos_hoje)} Evento(s) Hoje!",
             "#B22222", "#FFFFFF",
             lambda: app.selecionar_aba("Calendário")).pack(side="left")
    else:
