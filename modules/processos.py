...
def _resource_path(relative_path):
    """Caminho absoluto compatível com dev e executável PyInstaller."""
...
def _ic(path, size=(16, 16)):
    """Carrega e cacheia um CTkImage de assets/."""
    k = (path, size)
    if k not in _IC:
        try:
            _IC[k] = ctk.CTkImage(Image.open(_resource_path(f"assets/{path}")), size=size)
        except Exception:
            _IC[k] = None
    return _IC[k]
...
def _aplicar_icone(janela):
    ...
    try:
        ...
        import os
        # O ícone fica na pasta raiz do projeto (acima de modules/)
        base = os.path.dirname(os.path.abspath(__file__))
        # Tentar pasta atual e pasta pai (caso esteja em modules/)
        candidatos = []
        for pasta in [base, os.path.dirname(base)]:
            for nome in ["archon_icon.png", "archon_icon.ico", "icon_backup.png"]:
...
def criar_aba_processos(app, parent_frame):
    ...
    grupos_abas = [
        ...
        ["Não Pagos", "Pagos"],
        ["Certidão Não-Emitida", "Certidão Solicitada", "Certidão Emitida"],
        ["Cancelados"]
    ]
    ...
    app.var_st_certsolicitada = ctk.StringVar(value="")
    ...
    def regra_certsolicitada():
        if app.var_st_certsolicitada.get():
            app.var_st_certnao.set("Certidão Não-Emitida")
    ...
    ctk.CTkCheckBox(frame_status, text="Certidão Solicitada", font=ctk.CTkFont(weight="bold"),
                    variable=app.var_st_certsolicitada, onvalue="Certidão Solicitada", offvalue="",
                    command=regra_certsolicitada).grid(row=1, column=2, padx=12, pady=8, sticky="w")
    ctk.CTkCheckBox(frame_status, text="Certidão Emitida", font=ctk.CTkFont(weight="bold"),
                    ...
                    command=regra_certsim).grid(row=2, column=2, padx=12, pady=8, sticky="w")
...
def salvar_processo(app):
    ...
    # Verificar CNJ duplicado — apenas em novos cadastros (não em edições)
    id_edicao_atual = getattr(app, "id_edicao_proc", None)
    if not id_edicao_atual:
        processos_existentes = buscar_processos()
        if any(p[2] == acao for p in processos_existentes):
            app.lbl_erro_proc.configure(text="ERRO: Já existe um processo cadastrado com este N° de Ação (CNJ).")
            return
...
def atualizar_lista_processos(app, event=None):
    ...
    img_id           = _ic("icon_id.png")
    img_rg           = _ic("icon_rg.png")
    img_tel          = _ic("icon_tel.png")
    img_mail         = _ic("icon_mail.png")
    img_cnpj         = _ic("icon_cnpj.png")
    img_resp         = _ic("icon_responsavel.png")
    img_razao        = _ic("icon_razao.png",        (14,14))
    img_atuacao      = _ic("icon_atuacao.png",      (14,14))
    img_cliente_item = _ic("icon_cliente_item.png", (16,16))
    img_link         = _ic("icon_link.png",         (18,18))
    img_edit         = _ic("icon_edit.png",         (14,14))
    img_del          = _ic("icon_delete.png",       (14,14))
    img_seta         = _ic("icon_seta.png",         (12,12))
    img_seta_cima    = _ic("icon_seta_cima.png",    (12,12))
    img_alerta_proc  = _ic("icon_notificacoes_alerta.png", (16,16))
    img_clipe        = _ic("icon_clipe.png",        (14,14))
    ...
    # Buscar todos os clientes e associacoes UMA VEZ antes do loop
    # Elimina N queries por card — maior ganho de performance
    try:
        from database.database import buscar_clientes as _buscar_clientes
        todos_clientes = _buscar_clientes()
    except: todos_clientes = []
    
    try:
        from database.database import conectar as _conectar
        _conn = _conectar()
        _cur = _conn.cursor()
        _cur.execute("SELECT acao_principal, acao_associada FROM processos_associados")
        _assoc_rows = _cur.fetchall()
        _conn.close()
        assoc_map = {}
        for ap, aa in _assoc_rows:
            assoc_map.setdefault(ap, []).append(aa)
    except: assoc_map = {}

    try:
        from database.database import conectar as _conectar2
        _conn2 = _conectar2()
        _cur2 = _conn2.cursor()
        _cur2.execute("SELECT p.num_acao, e.id FROM estrategias e INNER JOIN processos p ON e.processo_id = p.id")
        estrat_map = dict(_cur2.fetchall())
        _conn2.close()
    except: estrat_map = {}
    ...
    def criar_card_proc(app, scroll_parent, p_dados):
        ...
        if nome != "[Sem Cliente]":
            if nome not in cache_clientes:
                cache_clientes[nome] = next(
                    (cl for cl in todos_clientes if str(cl[2]).strip().lower() == str(nome).strip().lower()),
                    None
                )
        ...
        associados_db = assoc_map.get(acao, [])
        ...
        id_estrat_vinc = estrat_map.get(acao)

        _img_est = _ic("edit_strat.png" if id_estrat_vinc else "criar_strat.png", (18,18))
        ...
        todos_processos_db = processos  # ja temos tudo em memoria
            ...
            for a_acao in associados_db:
                ...
                try:
                    ...
                    if proc_assoc_dados:
                        ...
                        cliente_assoc = next((cx for cx in todos_clientes if cx[2] == nome_cliente_assoc), None)
                        if cliente_assoc:
    ...
    for p in processos:
        ...
        if "Cancelado" in abas_do_processo:
            ...
            if "Certidão Solicitada" in abas_do_processo and "Certidão Não-Emitida" not in abas_do_processo:
                abas_do_processo.append("Certidão Não-Emitida")
            if "Certidão Emitida" in abas_do_processo:
                abas_do_processo = [a for a in abas_do_processo if a not in ("Certidão Não-Emitida", "Certidão Solicitada")]
...
def abrir_opcoes_processo(app, dados_proc):
    ...
    var_m_certsolicitada = ctk.StringVar(value="Certidão Solicitada" if "Certidão Solicitada" in status_list else "")
    ...
    def salvar_modal():
        sts = [var_m_andamento.get(), var_m_concluido.get(), var_m_pago.get(), var_m_naopago.get(), var_m_certsim.get(), var_m_certnao.get(), var_m_certsolicitada.get(), var_m_cancelado.get()]
    ...
    def r_csm():
        if var_m_certsim.get():
            ...
            var_m_cancelado.set("")
    def r_csol():
        if var_m_certsolicitada.get():
            var_m_certnao.set("Certidão Não-Emitida")
            var_m_certsim.set("")
            var_m_cancelado.set("")
    def r_cna():
    ...
    ctk.CTkCheckBox(frame_chk, text="Cert. Solicitada", font=ctk.CTkFont(weight="bold"), variable=var_m_certsolicitada, onvalue="Certidão Solicitada", offvalue="", command=r_csol).grid(row=1, column=2, padx=10, pady=5, sticky="w")
    ctk.CTkCheckBox(frame_chk, text="Cert. Emitida", font=ctk.CTkFont(weight="bold"), variable=var_m_certsim, onvalue="Certidão Emitida", offvalue="", command=r_csm).grid(row=2, column=2, padx=10, pady=5, sticky="w")
    ...
    def ir_para_editar():
        ...
        app.var_st_certsolicitada.set("Certidão Solicitada" if "Certidão Solicitada" in status_list else "")
