...
# sys.executable pode vir sem caminho absoluto dependendo de como o Windows
# lançou o processo (CWD pode ser System32, etc.).
# GetModuleFileNameW é a única API que SEMPRE retorna o caminho real do exe.

def _resolver_pasta_exe() -> str:
    """Retorna a pasta do .exe de forma 100% confiável via WinAPI."""
    if not getattr(sys, 'frozen', False):
        # Desenvolvimento: sobe dois níveis  database/database.py → raiz
        return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

    # 1ª tentativa: GetModuleFileNameW (mais confiável)
    try:
        import ctypes
        buf = ctypes.create_unicode_buffer(32768)
        ctypes.windll.kernel32.GetModuleFileNameW(None, buf, len(buf))
        pasta = os.path.dirname(buf.value)
        if pasta:
            return pasta
    except Exception:
        pass

    # 2ª tentativa: abspath de sys.executable
    return os.path.dirname(os.path.abspath(sys.executable))


_BASE_DIR = _resolver_pasta_exe()
os.makedirs(_BASE_DIR, exist_ok=True)
DB_PATH    = os.path.join(_BASE_DIR, "advocacia.db")

# ── Log de diagnóstico (apague após confirmar que funciona) ───────────────────
try:
    import tempfile, datetime as _dt
    _log = os.path.join(tempfile.gettempdir(), "archon_debug.txt")
    with open(_log, "a", encoding="utf-8") as _f:
        _f.write(f"\n[{_dt.datetime.now()}]\n")
        _f.write(f"  frozen        = {getattr(sys, 'frozen', False)}\n")
        _f.write(f"  sys.executable= {sys.executable!r}\n")
        _f.write(f"  _BASE_DIR     = {_BASE_DIR!r}\n")
        _f.write(f"  DB_PATH       = {DB_PATH!r}\n")
except Exception:
    pass
# ─────────────────────────────────────────────────────────────────────────────
